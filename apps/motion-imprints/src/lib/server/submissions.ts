...
export function getRepository(): Repository | null {
  ...
  if (url) {
    ...
    const pool = g.motionPool;
    return createRepository(() => pool.connect(), makeReference);
  }
  if (
    process.env.NODE_ENV !== "production" &&
    process.env.QUOTE_STORE === "memory"
  ) {
    g.motionMemoryRepo ??= createMemoryRepository(makeReference);
    return g.motionMemoryRepo;
  }
  return null;
}
