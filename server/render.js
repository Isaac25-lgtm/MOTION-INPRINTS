 ...
 * It builds a database client and administrator configuration from
 * `serverConfig()` at module scope, so importing it here unconditionally made
 * merely importing the bridge require DATABASE_URL — which crashed the whole
 * test suite on a machine with no environment configured, before the bridge's
 * own tests could run. The bridge translates Node to Fetch and knows
